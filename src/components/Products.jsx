...
export const Products = () => {
    ...
    useEffect(() => {
        setBestSellers(['All', 'Oversize', 'Indie', 'Nuevo']);
    ...
    const filterProducts = (filter) => {
        if (filter === 'All') {
            ...
            const filteredProducts = productsJson.filter(product => product.category === filter);
                ...
                <ul className="filter-list">
                    {
                        bestSellers.map((bestSeller, index) => (
                            <li key={index}>
                                <button className={`filter-btn ${bestSeller === selectedFilter ? 'active' : ''}`} onClick={() => filterProducts(bestSeller)}>{bestSeller}</button>
                            </li>
                        ))
                    }
                </ul>
...
<figure className="card-banner">
    ...
    <ul className="card-action-list">
        ...
        <li className="card-action-item">
            <button className="card-action-btn" aria-labelledby={`card-label-1-${index}`} onClick={() => navigate(`/products/${product.id}`)}>
